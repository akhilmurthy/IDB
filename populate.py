...
def scrapePlayer():
	...
	for h in range(0,len(players)):
		...
		try:
			...
			newakhilstring = newakhilstring + str.join(',', (str(h+1), str("\'" + name + "\'"), str(skill_rank), '\'grandmaster\'', str(win_rate), str(level), str(primary_hero_id)))
			newakhilstring = newakhilstring + "),"
			
			#download icon
			for pdiv in soup.findAll('div', {'class':'profile-info'}):
				for idiv in soup.findAll('div', {'class':'img'}):
					for img in soup.findAll('img'):
						iconurl = img['src']
						if 'unlocks' in iconurl:
							print(iconurl)
							req = urllib.request.Request(iconurl, headers={'User-Agent': 'Mozilla/5.0'})
							theimage = urllib.request.urlopen(req, context=context)

							output = open("static/media/players/"+ str(h+1) + ".png", 'wb')
							output.write(theimage.read())
							output.close()
			
			'''
			#attempt to add directly to db
